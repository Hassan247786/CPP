...
class FourMomentum {
  private: std::unique_ptr < std::vector < double >> momentum_vector;
  public: FourMomentum(double input_energy, double input_momentum_x, double input_momentum_y, double input_momentum_z): momentum_vector(std::
    make_unique < std::vector < double >> (4)) {
  ...
  void set_momentum_x(double input_momentum_x) {
    ( * momentum_vector)[1] = input_momentum_x;
  }
  void set_momentum_y(double input_momentum_y) {
    ( * momentum_vector)[2] = input_momentum_y;
  }
  void set_momentum_z(double input_momentum_z) {
    ( * momentum_vector)[3] = input_momentum_z;
  }
  double get_energy() const {
    return ( * momentum_vector)[0] * speed_of_light;
  }
  double get_momentum_x() const {
    return ( * momentum_vector)[1];
  }
  double get_momentum_y() const {
    return ( * momentum_vector)[2];
  }
  double get_momentum_z() const {
    return ( * momentum_vector)[3];
  }
...
class Particle {
  ...
  public: Particle(double mass, double energy, double px, double py, double pz): mass {
    mass
  },
  momentum {
    std::make_unique < FourMomentum > (energy, px, py, pz)
  } {}
  virtual void print_particle_data() const = 0;
  virtual int get_lepton_number() const {
    return 0;
  }
  ...
  virtual std::string get_quark_flavour() const {
    return 0;
  }
  virtual double get_energy() {
    return momentum -> get_energy();
  }
  FourMomentum & get_momentum() {
    return * momentum;
  }
  virtual~Particle() {}
};
class Lepton: public Particle {
  ...
  bool anti_particle;
  //true if antiparticle present
  int charge;
  public: Lepton(double mass, double input_spin, double energy, double px, double py, double pz, int input_charge, bool anti_particle = false): Particle(mass, energy, px, py, pz),
  spin {
    input_spin
  },
  anti_particle {
  ...
  charge {
    anti_particle ? input_charge : input_charge * (-1)
  } {
    ...
    if (std::abs(invariant_mass - mass) > 1e-3) {
      ...
      //rearranged formula for invariant mass
  ...
  int get_lepton_number() const override {
    return anti_particle ? -1 : 1;
  }
  //override abstract class virtual function
  ...
  double get_momentum_x() const {
    return momentum -> get_momentum_x();
  }
  double get_momentum_y() const {
    return momentum -> get_momentum_y();
  }
  double get_momentum_z() const {
    return momentum -> get_momentum_z();
  }
  void set_energy(double energy) {
    momentum -> set_energy(energy);
  }
  void set_momentum_x(double px) {
    momentum -> set_momentum_x(px);
  }
  void set_momentum_y(double py) {
    momentum -> set_momentum_y(py);
  }
  void set_momentum_z(double pz) {
    momentum -> set_momentum_z(pz);
  }
...
class Quark: public Particle {
  ...
  public: Quark(double mass, double input_spin, double energy, double px, double py, double pz,
    const std::string & flavour, bool anti_particle,
      const
        std::string & colour): Particle(mass, energy, px, py, pz),
  spin {
    input_spin
  },
  anti_particle {
  ...
  colour_charge {
    ...
    if ((anti_particle == true && (colour_charge.compare(0, 4, start_of_string) != 0)) ||
      //comparing expected and required strings
      (anti_particle == false && (colour_charge.compare(0, 4, start_of_string) == 0)))
    //strings are equal
    {
  ...
  std::string get_quark_flavour() const override {
    return quark_flavour;
  }
  void set_energy(double energy) {
    momentum -> set_energy(energy);
  }
...
class Boson: public Particle {
  ...
  public: Boson(double mass, double input_spin, double energy, double px, double py, double pz,
    const std::string & input_boson_type, bool
    anti_particle = false): Particle(mass, energy, px, py, pz),
  spin {
    input_spin
  },
  anti_particle {
...
class HiggsBoson: public Particle {
  private: std::vector < std::unique_ptr < Particle >> higgs_decay_products;
  public: HiggsBoson(double mass, double energy, double px, double py, double pz): Particle(mass, energy, px, py, pz) {
    double invariant_mass = momentum -> compute_invariant_mass();
    if (std::abs(invariant_mass - mass) > 1e-3)
  ...
  //does not need overriding in this case
  void set_energy(double energy) {
    momentum -> set_energy(energy);
  }
  void set_momentum_x(double px) {
...
class Calorimeter {
  ...
  double total_calorimeter_energy() const {
    return layer_1 + layer_2 + layer_3 + layer_4;
  }
};
class Electron: public Lepton {
  private: Calorimeter calorimeter;
  public: Electron(double mass, double energy, double px, double py, double pz, bool anti_particle): Lepton(mass, 0.5, energy, px, py, pz, -1, anti_particle ? 1 : -1) {}
  int get_lepton_number() const override {
    return anti_particle ? -1 : 1;
  }
  std::string get_particle_type() const override {
...
class Muon: public Lepton {
  private: bool muon_isolation;
  public: Muon(double mass, double energy, double px, double py, double pz, bool anti_particle, bool input_muon_isolation): Lepton(mass, 105.7, energy, px, py, pz, -1, anti_particle),
  muon_isolation {
    input_muon_isolation
  } {}
  void set_isolation_status(bool isolation_status) {
    muon_isolation = isolation_status;
  }
  bool get_isolation_status() const {
    return muon_isolation;
  }
  std::string get_particle_type() const override {
    return "Muon";
  }
...
class ElectronNeutrino: public Lepton {
  private: bool has_interacted;
  public: ElectronNeutrino(double mass, double energy, double px, double py, double pz, bool anti_particle, bool interaction_status): Lepton(mass, 0.5, energy, px, py, pz, 0, anti_particle),
  has_interacted {
    interaction_status
  } {}
...
class MuonNeutrino: public Lepton {
  private: bool has_interacted;
  public: MuonNeutrino(double mass, double energy, double px, double py, double pz, bool anti_particle, bool interaction_status): Lepton(mass, 0.5, energy, px, py, pz, 0, anti_particle),
  has_interacted {
    interaction_status
  } {}
  bool get_interaction_status() const {
...
class TauNeutrino: public Lepton {
  private: bool has_interacted;
  public: TauNeutrino(double mass, double energy, double px, double py, double pz, bool anti_particle, bool interaction_status): Lepton(mass, 0.5, energy, px, py, pz, 0, anti_particle),
  has_interacted {
    interaction_status
  } {}
  bool get_interaction_status() const {
...
class Photon: public Boson {
  public: Photon(double mass, double energy, double px, double py, double pz, bool anti_particle = false): Boson(mass, 1, energy, px, py, pz, "Photon", anti_particle) {}
  std::string get_particle_type() const override {
    return "Photon";
  }
  double get_charge() const {
...
class Gluon: public Boson {
  private: std::string colour_charge;
  std::string anticolour_charge;
  public: Gluon(double mass, double energy, double px, double py, double pz, bool anti_particle,
    const std::string & input_colour,
      const std::string &
        input_anticolour): Boson(mass, 1, energy, px, py, pz, "Gluon", anti_particle),
  colour_charge {
    input_colour
  },
  anticolour_charge {
    ...
    std::string allowed_colour_charges[3] = {
      "red",
      "blue",
      "green"
    };
    std::string allowed_anticolour_charges[3] = {
      "antired",
      "antiblue",
      "antigreen"
    };
    ...
    for (int i = 0; i < 3; i++)
    //iterating through lists above to check colour charges
    {
...
class ZBoson: public Boson {
  public: ZBoson(double mass, double energy, double px, double py, double pz, bool anti_particle = false): Boson(mass, 1, energy, px, py, pz, "Z", anti_particle) {}
  std::string get_particle_type() const override {
    return "ZBoson";
  }
  double get_spin() const {
...
class WPlusBoson: public Boson {
  public: WPlusBoson(double mass, double energy, double px, double py, double pz, bool anti_particle = false): Boson(mass, 1, energy, px, py, pz, "W+", anti_particle) {}
  std::string get_particle_type() const override {
    return "WPlusBoson";
  }
  double get_spin() const {
...
class WMinusBoson: public Boson {
  public: WMinusBoson(double mass, double energy, double px, double py, double pz, bool anti_particle = false): Boson(mass, 1, energy, px, py, pz, "W-", anti_particle) {}
  std::string get_particle_type() const override {
    return "WMinusBoson";
  }
  double get_charge() const override {
...
class UpQuark: public Quark {
  public: UpQuark(double mass, double energy, double px, double py, double pz, bool anti_particle,
    const std::string & colour): Quark(mass, 0.5, energy, px, py, pz, "up", anti_particle, colour) {}
  std::string get_particle_type() const override {
    return "UpQuark";
  }
  double get_charge() const override {
    return anti_particle ? -2 / 3 : 2 / 3;
  }
  double get_spin() const {
    return 0.5;
  }
  double get_baryon_number() const override {
    return anti_particle ? -1 / 3 : 1 / 3;
  }
};
class DownQuark: public Quark {
  public: DownQuark(double mass, double energy, double px, double py, double pz, bool anti_particle,
    const std::string & colour): Quark(mass, 0.5, energy, px, py, pz, "down", anti_particle, colour) {}
  std::string get_particle_type() const override {
    return "DownQuark";
  }
  double get_charge() const override {
...
class Charm: public Quark {
  public: Charm(double mass, double energy, double px, double py, double pz, bool anti_particle,
    const std::string & colour): Quark(mass, 0.5, energy, px, py, pz, "charm", anti_particle, colour) {}
  std::string get_particle_type() const override {
    return "CharmQuark";
  }
  double get_charge() const override {
...
class Strange: public Quark {
  public: Strange(double mass, double energy, double px, double py, double pz, bool anti_particle,
    const std::string & colour): Quark(mass, 0.5, energy, px, py, pz, "strange", anti_particle, colour) {}
  std::string get_particle_type() const override {
    return "StrangeQuark";
  }
  double get_charge() const override {
...
class Top: public Quark {
  public: Top(double mass, double energy, double px, double py, double pz, bool anti_particle,
    const std::string & colour): Quark(mass, 0.5, energy, px, py, pz, "top", anti_particle, colour) {}
  std::string get_particle_type() const override {
    return "TopQuark";
  }
  double get_charge() const override {
...
class Bottom: public Quark {
  public: Bottom(double mass, double energy, double px, double py, double pz, bool anti_particle,
    const std::string & colour): Quark(mass, 0.5, energy, px, py, pz, "bottom", anti_particle, colour) {}
  std::string get_particle_type() const override {
    return "BottomQuark";
  }
  double get_charge() const override {
...
class Tau: public Lepton {
  private: std::vector < std::unique_ptr < Particle >> tau_decay_products;
  public: Tau(double mass, double energy, double px, double py, double pz, bool anti_particle): Lepton(mass, 0.5, energy, px, py, pz, anti_particle ? 1 : -1, anti_particle) {
...
class Catalogue {
  private: std::list < std::shared_ptr < Particle >> catalogue_particles;
  //wrap particle catalogue in an STL list
  int particle_count = 0,
  higgs_boson_count = 0,
  electron_count = 0,
  muon_count = 0,
  tau_count = 0,
  electron_neutrino_count = 0,
  muon_neutrino_count = 0,
  tau_neutrino_count = 0,
  up_quark_count = 0,
  down_quark_count = 0,
  charm_quark_count = 0,
  strange_quark_count = 0,
  top_quark_count = 0,
  bottom_quark_count = 0,
  gluon_count = 0,
  photon_count = 0,
  w_minus_count = 0,
  w_plus_count = 0,
  z_boson_count = 0;
  public: void add_particle_to_catalogue(std::unique_ptr < Particle > particle) {
  ...
  int total_number_catalogue_particles() const {
    return particle_count;
  }
  void particles_sorted_by_descending_energy()
  //lambda function to sort particles by energy
  {
    catalogue_particles.sort([](const std::shared_ptr < Particle > & particle_one,
      const std::shared_ptr < Particle > & particle_two) {
  ...
  std::vector < double > combined_catalogue_momentum() const {
    std::vector < double > combined_momentum = {
      0,
      0,
      0,
      0
...
template < typename particle_class_one, typename particle_class_two >
  bool particle_energy_comparison(const particle_class_one & particle1,
    const particle_class_two & particle2) {
    return particle1.get_energy() < particle2.get_energy();
  }
int main() { //Testing calorimeter
