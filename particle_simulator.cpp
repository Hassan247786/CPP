...
class FourMomentum {
  ...
  public: FourMomentum(double input_energy, double input_momentum_x, double input_momentum_y, double input_momentum_z): momentum_vector {
    std::make_unique < std::vector < double >> (4)
  } {
...
class lepton {
  ...
  lepton(double input_mass, int input_charge, double input_energy, double input_momentum_x,
    double input_momentum_y, double input_momentum_z): rest_mass {
    input_mass
  },
  charge {
    input_charge
  ...
  momentum {
    std::make_unique < FourMomentum > (input_energy,
      input_momentum_x, input_momentum_y, input_momentum_z)
  } {}
  const FourMomentum & get_momentum() const {
    return * momentum;
  }
  ...
  friend double momentum_dot_product(const FourMomentum & particle_one_momentum,
    const FourMomentum & particle_two_momentum);
  virtual~lepton() =
  default;
...
class Calorimeter {
  ...
  double total_calorimeter_energy() const {
    return EM1 + EM2 + HAD1 + HAD2;
  }
};
class Electron: public lepton {
  private: Calorimeter calorimeter;
  std::vector < double > lepton_energies;
  public: Electron(double input_energy, double input_momentum_x, double input_momentum_y, double input_momentum_z,
    const std::vector < double > & energies): lepton(0.511, -1, input_energy, input_momentum_x, input_momentum_y, input_momentum_z),
  lepton_energies {
    energies
  } {
    ...
    if (std::abs(get_momentum().get_energy() - calorimeter.total_calorimeter_energy()) > 1e-4)
      std::cout << "The total energy deposited by the electron in the calorimeter does not match the electron's four momentum energy. " << std::
      endl;
  }
  std::vector < double > get_lepton_energies() const {
    return lepton_energies;
  }
  friend void print_electron_data(const Electron & electron_particle);
};
class Muon: public lepton {
  private: bool muon_isolation;
  public: Muon(double input_energy, double input_momentum_x, double input_momentum_y, double input_momentum_z, bool isolation_status): lepton(105.7, -1, input_energy, input_momentum_x, input_momentum_y, input_momentum_z),
  muon_isolation {
    isolation_status
  } {}
...
class Neutrino: public lepton {
  ...
  std::string flavour;
  public: Neutrino(double input_energy, double input_momentum_x, double input_momentum_y, double input_momentum_z,
    std::string input_flavour, bool interaction_status): lepton(0, 0, input_energy, input_momentum_x, input_momentum_y, input_momentum_z),
  flavour {
    input_flavour
  },
  hasInteracted {
  ...
  void set_interaction_status(bool interaction_status) {
    hasInteracted = interaction_status;
  }
  bool get_interaction_status() const {
    return hasInteracted;
  }
  const std::string & get_flavour() const {
    return flavour;
  }
  friend void print_neutrino_data(const Neutrino & neutron_particle);
};
class Tau: public lepton {
  private: bool leptonic_decay;
  std::vector < std::shared_ptr < lepton >> decay_products;
  public: Tau(double input_energy, double input_momentum_x, double input_momentum_y, double input_momentum_z, bool leptonic_or_hadronic): lepton(1777, -1, input_energy, input_momentum_x, input_momentum_y, input_momentum_z),
  leptonic_decay {
    leptonic_or_hadronic
  } {}
  void add_decay_product(const std::shared_ptr < lepton > & product) {
    decay_products.push_back(product);
  }
  bool get_is_leptonic() const {
...
std::vector < double > sum_of_momentum(const FourMomentum & particle_one_momentum,
  const FourMomentum & particle_two_momentum) {
...
double momentum_dot_product(const FourMomentum & particle_one_momentum,
  const FourMomentum & particle_two_momentum) {
...
int main() {
  ...
  //example energy values (just for test)
  std::unique_ptr < Electron > new_electron_pointer {
    new Electron(50, 3, 2, 0, lepton_energies)
  };
  ...
  std::shared_ptr < Tau > tau_pointer {
    new Tau(2, 0.5, 0, 0, true)
  };
