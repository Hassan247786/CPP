...
int main() {
  ...
  if (user_sorting_preference == 1) {
    sort(filtered_data.begin(), filtered_data.end(), [](const auto & first_tuple,
      const auto & second_tuple) {
  ...
  for (std::vector < std::tuple < double, int, std::string >> ::iterator vector_iterator = vector_begin; vector_iterator != vector_end; ++
    vector_iterator) {
